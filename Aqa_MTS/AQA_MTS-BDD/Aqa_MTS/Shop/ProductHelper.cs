...
internal static class ProductHelper
{
    ...
    {
        if (items != null && items.TryGetValue(id, out var product))
        {
            items.Remove(id);
        }
        else
    ...
    {
        if (inventory != null && !string.IsNullOrEmpty(id) && inventory.TryGetValue(id, out var product))
        {
    ...
    public static void SearchByID(Dictionary<string, Product>? items, string? id)
    {
        ...
        if (items.Count != 0 || !string.IsNullOrEmpty(id))
        {
            if (items.TryGetValue(id, out value))
            {
             ...
             //   {
                    Console.WriteLine(
                        $"ID {id}: {value.NameProduct} цена: {value.PriceProduct}р. кол-во: {value.QuantityProduct}");
              //  }
            }
            else
            {
